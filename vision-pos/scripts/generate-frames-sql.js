const fs = require('fs');
const path = require('path');

const csv = fs.readFileSync(path.join(__dirname, '../db-backup-20251211/frames.csv'), 'utf8');
const lines = csv.split('\n');
const headers = lines[0].split(',');

// Quote headers for PostgreSQL (camelCase columns need quotes)
const quotedHeaders = headers.map(h => `"${h}"`).join(',');

const batchSize = 50;
const batches = [];
let currentBatch = [];

for (let i = 1; i < lines.length; i++) {
  if (!lines[i].trim()) continue;

  const values = [];
  let current = '';
  let inQuotes = false;

  for (const char of lines[i]) {
    if (char === '"') {
      inQuotes = !inQuotes;
    } else if (char === ',' && !inQuotes) {
      values.push(current);
      current = '';
    } else {
      current += char;
    }
  }
  values.push(current);

  const sqlValues = values.map((v, idx) => {
    const header = headers[idx];
    if (v === '' || v === 'null') return 'NULL';
    if (v === 't') return 'true';
    if (v === 'f') return 'false';

    // Numeric fields - don't quote
    if (['wholesaleCost', 'retailPrice', 'stockQuantity', 'eyeSize', 'bridge', 'temple', 'posDisplayOrder'].includes(header)) {
      return v || 'NULL';
    }

    // Everything else is a string
    return "'" + v.replace(/'/g, "''") + "'";
  });

  currentBatch.push('(' + sqlValues.join(',') + ')');

  if (currentBatch.length >= batchSize) {
    batches.push(currentBatch);
    currentBatch = [];
  }
}

if (currentBatch.length > 0) {
  batches.push(currentBatch);
}

// Output as JSON array of SQL statements with quoted headers
const sqlStatements = batches.map(batch =>
  'INSERT INTO frames (' + quotedHeaders + ') VALUES ' + batch.join(',\n') + ';'
);

console.log(JSON.stringify(sqlStatements, null, 2));
console.error('Generated ' + sqlStatements.length + ' batch statements for ' + (lines.length - 1) + ' records');
